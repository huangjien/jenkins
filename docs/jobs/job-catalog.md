...
| wst | Pipeline | `jobs/templates/pipeline_job_wst_legacy_disabled.groovy` | App Team | Deprecated legacy job; seed enforces disabled state and points users to `WST-CI-Pipeline` |
| website-ci-cd | Pipeline | `jobs/templates/pipeline_job_website.groovy` + `pipelines/apps/website.Jenkinsfile` | App Team | Seed-managed replacement for the legacy inline website job; runs on demand, tracks the last successful upstream website SHA in build metadata, runs pnpm quality gates, builds `huangjien/website-edge`, and optionally deploys Cloud Run |
