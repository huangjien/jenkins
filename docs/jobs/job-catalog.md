...
| website-ci-cd | Pipeline | `jobs/templates/pipeline_job_website.groovy` + `pipelines/apps/website.Jenkinsfile` | App Team | Seed-managed replacement for the legacy inline website job; polls website main every 5 minutes (cron + git ls-remote), runs pnpm quality gates, builds `huangjien/website-edge`, deploys Cloud Run |
